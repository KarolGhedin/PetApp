    ...
    return (
        <section>
            <div className={styles.pet_home_header}>
                <h1>Adopt a Pet</h1>
                <p>See the details of each pet and know his tutor</p>
            </div>
            ...
            {pets.length > 0 && 
                pets.map((pet) => (
                    <div className={styles.pet_card}>
                        ...
                        {pet.available ? ( 
                            <Link to={`pet/${pet._id}`}>Details</Link> ): (<p className={styles.adopted_text}>Adopted</p>
                        )}
                    </div>
                ))}
            {pets.length === 0 && (
                <p>There's none pet to adoption right now</p>
            )}
        </div>
        </section>
    )
